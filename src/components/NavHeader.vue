  ...
  <header class="header">
    ...
    <div class="navbar">
      <div class="navbar-left-container">
        <a href="/">
          <img class="navbar-brand-logo" src="./../assets/img/logo.jpg"></a>
      </div>
          ...
          <div class="confirm-tips">
            ...
            <ul>
              <li class="regi_form_input">
                ...
                <input type="text" tabindex="1" name="loginname" v-model="userName" class="regi_login_input regi_login_input_left" placeholder="用户名称" data-type="loginname">
              </li>
              <li class="regi_form_input noMargin">
                <i class="icon IconPwd"></i>
                <input type="password" tabindex="2" name="password" v-model="userPwd" class="regi_login_input regi_login_input_left login-input-no input_text" placeholder="登录密码" @keyup.enter="login">
              </li>
            </ul>
          </div>
          <div class="login-wrap">
            <a href="javascript:;" class="btn-login" @click="login" @keyup.enter="login">登 录</a>
          </div>
        </div>
      </div>
    </div>
    ...
    checkLogin() {
      ...
      this.Axios.get("/users/checkLogin").then(res => {
        if (res.data.status == 0) {
          this.$store.commit("updateUserName", res.data.result.userName);
          this.getCartCount();
        }
      });
    },
